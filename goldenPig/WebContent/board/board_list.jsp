<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>질문과 답변(Q&A)</title>
<link rel="stylesheet" href="../assets/css/board/board_list.css">
</head>
<body>
	<div class="wrap">
		<div class="body-flex">


			<!-- ----------------------- 첫번째 게시물 -----------------------------  -->


			<div class="content-wrap">
				<div class="content">
					<div class="tabs-wrap">
						<div class="tabs">
							<button class="active">
								<span class="bullet">추가 답변하기</span>
							</button>
							<button class="active">
								<span class="bullet color-#989898">빠른 답변하기</span>
							</button>
							<hr class="tabs-line">
						</div>
					</div>
					<div class="content-body">
						<div class="question-card">
							<article class="card">
								<div class="card-main">
									<header class="card-header">
										<a href="javascript:void(0)">
											<h1 class="card-header-title">연말정산 월세 공제 관련해서 질문이요.</h1>
										</a>
									</header>
									<div class="card-content">
										<div class="card-footer">
											<div class="flex-justify-between">
												<div class="flex-items-center">
													<span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 1
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span>
												</div>
												<span class="card-header-infomation">2023. 02. 16.
													23:06</span>
											</div>
										</div>
									</div>
								</div>
							</article>
						</div>
					</div>


					<!-- ----------------------- 두번째 게시물 -----------------------------  -->


					<div class="content-body">
						<div class="question-card">
							<article class="card">
								<div class="card-main">
									<header class="card-header">
										<a href="javascript:void(0)">
											<h1 class="card-header-title">연말정산 월세 공제 관련해서 질문이요.</h1>
										</a>
									</header>
									<div class="card-content">
										<div class="card-footer">
											<div class="flex-justify-between">
												<div class="flex-items-center">
													<span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 1
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span>
												</div>
												<span class="card-header-infomation">2023. 02. 16.
													23:06</span>
											</div>
										</div>
									</div>
								</div>
							</article>
						</div>
					</div>


					<!-- ----------------------- 세번째 게시물 -----------------------------  -->


					<div class="content-body">
						<div class="question-card">
							<article class="card">
								<div class="card-main">
									<header class="card-header">
										<a href="javascript:void(0)">
											<h1 class="card-header-title">연말정산 월세 공제 관련해서 질문이요.</h1>
										</a>
									</header>
									<div class="card-content">
										<div class="card-footer">
											<div class="flex-justify-between">
												<div class="flex-items-center">
													<span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 1
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span>
												</div>
												<span class="card-header-infomation">2023. 02. 16.
													23:06</span>
											</div>
										</div>
									</div>
								</div>
							</article>
						</div>
					</div>


					<!-- ----------------------- 네번째 게시물 -----------------------------  -->


					<div class="content-body">
						<div class="question-card">
							<article class="card">
								<div class="card-main">
									<header class="card-header">
										<a href="javascript:void(0)">
											<h1 class="card-header-title">연말정산 월세 공제 관련해서 질문이요.</h1>
										</a>
									</header>
									<div class="card-content">
										<div class="card-footer">
											<div class="flex-justify-between">
												<div class="flex-items-center">
													<span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 1
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span>
												</div>
												<span class="card-header-infomation">2023. 02. 16.
													23:06</span>
											</div>
										</div>
									</div>
								</div>
							</article>
						</div>
					</div>


					<!-- ----------------------- 다섯번째 게시물 -----------------------------  -->

					<div class="content-body">
						<div class="question-card">
							<article class="card">
								<div class="card-main">
									<header class="card-header">
										<a href="javascript:void(0)">
											<h1 class="card-header-title">연말정산 월세 공제 관련해서 질문이요.</h1>
										</a>
									</header>
									<div class="card-content">
										<div class="card-footer">
											<div class="flex-justify-between">
												<div class="flex-items-center">
													<span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 1
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span> <span class="card-label-icon"> <img class="star"
														src="../assets/img/board/star-img.png"> 0
													</span>
												</div>
												<span class="card-header-infomation">2023. 02. 16.
													23:06</span>
											</div>
										</div>
									</div>
								</div>
							</article>
						</div>
					</div>


					<!-- ----------------------- 오른쪽 사이드 바 -----------------------------  -->

				</div>
				<aside class="right-side-bar">
					<button class="write-board">나도 질문하기</button>
				</aside>
			</div>
		</div>
	</div>
</body>
</html>